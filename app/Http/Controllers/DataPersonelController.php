...
public function showProfile()
    {
        ...
        $personel = DB::table('data_personnels')->where('user_id', $userId)->first();

        // If personnel data is not found, redirect with an error message
        if (!$personel) {
            return redirect()->route('list.personel')->with('error', 'Profil tidak ditemukan!');
        }
